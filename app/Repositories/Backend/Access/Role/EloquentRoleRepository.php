...
class EloquentRoleRepository implements RoleRepositoryContract
{
    ...
    public function create($input)
    {
        ...
        $role->sort = isset($input['sort']) && strlen($input['sort']) && is_numeric($input['sort']) ? (int) $input['sort'] :0;
    ...
    public function update($id, $input)
    {
        ...
        //See if the has all access, administrator always has all access
        if($role->id == 1){
            $all = true;
        }else{
            $all = $input['associated-permissions'] == 'all' ? true : false;
        ...
        //The config is only required if all is false
        if(! $all){
            //See if the role must contain a permission as per config
            if(config('access.roles.role_must_contain_permission') && count($input['permissions']) == 0){
                throw new GeneralException(trans('exceptions.backend.access.roles.needs_permission'));
        ...
        if($role->save()){
            //If role has all access detach all permissions because they're not need
            if($all){
    ...
    public function destroy($id)
    {
        ...
        //Don't delete the role if there are users associated
        ...
        throw new GeneralException('exceptions.backend.access.roles.delete_error');
    }
    ...
    public function getDefaultUserRole()
    {
        if(is_numeric(config('access.users.default_role'))){
            Role::where('id', (int)config('access.users.default_role'))->first();
        }
