...
class EloquentPermissionRepository implements PermissionRepositoryContract
{
    ...
    {
        if($withRoles){
            return Permission::with('roles', 'dependencies.permission')->orderBy($order_by,$sort)->all();
        }

        return Permission::with('dependencies.permission')->orderBy($order_by,$sort)->all();
    }
