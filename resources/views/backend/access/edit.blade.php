...
@section('content')
    ...
    <div class="box box-success">
        ...
        <div class="box-body">
            ...
            @if($user->id != 1)
                <div class="form-group">
                    <label for="" class="col-lg-2 control-label">{{ trans('validation.attributes.backend.access.users.active') }}</label>
                    <div class="col-lg-1">
                ...
                <div class="form-group">
                    <label for="" class="col-lg-2 control-label">{{ trans('validation.attributes.backend.access.users.confirmed') }}</label>
                    <div class="col-lg-1">
                ...
                <div class="form-group">
                    <label for="" class="col-lg-2 control-label">{{ trans('validation.attributes.backend.access.users.associated_roles') }}</label>
                    <div class="col-lg-3">
                        @if(count($roles) > 0)
                            @foreach($roles as $role)
                                <input type="checkbox" value="{{$role->id}}" name="assignees_roles[]" {{in_array($role->id, $user_roles) ? 'checked':''}} id="role-{{$role->id}}" />
                                <label for="" id="role-{{$role->id}}">{{ $role->name }}</label>
                                <a href="#" data-role="role-{{$role->id}}" class="show-permissions small">
                                    (
                                ...
                                <br />
                                <div class="permission-list hidden" data-role="role-{{$role->id}}">
                                    @if($role->all)
                ...
                <div class="form-group">
                    <label for="" class="col-lg-2 control-label">{{ trans('validation.attributes.backend.access.users.other_permissions') }}</label>
                    <div class="col-lg-10">
                        <div class="alert alert-info">
                            <i class="fa fa-info-circle"></i>{{ trans('validation.attributes.backend.access.users.permission_check') }}
                        </div>
                                ...
                                <div class="col-lg-4">
                                    <ul style="margin:0;padding:0;list-style: none;">
                                        @foreach($perm as $p)
                                            ...
                                            ?>
                                            <li>
                                                <input type="checkbox" value="{{$p['id']}}" name="permission_user[]" data-dependencies="{{ $dependencies }}" {{ in_array($p['id'], $user_permissions) ? 'checked':'' }} id="permission-{{$p['id']}}">
